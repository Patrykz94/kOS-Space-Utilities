...
FUNCTION Prelaunch {
	IF subRunmode = 0 {			//	Request launch data from user
		// ADD the UI creation to get all the required data
		SET lT TO TIME:SECONDS + 10. // CHANGE THIS!!!
		AddUIMessage("Launching in " + (lT - mT) + " seconds!").
		SET subRunmode TO 1.
...
FUNCTION Launch {
	...
	IF launchGLimit <> 0 AND ShipTWR() > 0 {
		SET tval TO MIN(1, MAX(0.2, landing["launchGLimit"]/ShipTWR())).
	...
	IF subRunmode = 0 {	//	Wait until over 100 meters above ground before pitching over
		...
		LOCAL pitch IS MAX(5, MIN(90, 90 * (speedComponent * 0.5 + altitudeComponent * 0.5))).
...
FUNCTION Coasting {
	...
	IF subRunmode = 0 {
		...
		IF SHIP:APOAPSIS < targetApoapsis {
			SET tval TO MAX(0.05, MIN(MIN(1, landing["launchGLimit"]/ShipTWR()), (targetApoapsis - SHIP:APOAPSIS) / 2500)).
		} ELSE {
			SET tval TO 0.
			SET subRunmode TO 0.
			SET runmode TO 3.
...
FUNCTION Circularization {
	IF subRunmode = 0 {
		...
		LOCAL circDeltaV IS OrbitalVelocityAt(SHIP:APOAPSIS) - VELOCITYAT(SHIP, ETA:APOAPSIS).
